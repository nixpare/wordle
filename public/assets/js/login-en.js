...
loginForm.addEventListener('submit', ev => {
	...
	fetch('/en/login', {
		...
		if (response.status !== 200) {
			loginError.textContent = 'Email or Password wrong'
		...
		loginError.textContent = 'Error: retry later'
		loginError.classList.remove('hidden')
	})
...
registrationForm.addEventListener('submit', ev => {
	...
	let cred = {
		...
		password: registrationForm.querySelector('input[name="new-password"]').value,
		confirmPassword: registrationForm.querySelector('input[name="confirm-password"]').value
	}

	if (cred.password !== cred.confirmPassword) {
		registrationError.textContent = 'The passwords do not match'
	...
	fetch('/en/register', {
		...
		registrationError.textContent = 'Error: retry later'
		registrationError.classList.remove('hidden')
	})
