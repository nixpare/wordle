...
class Wordle {
	...
	async init() {
		const errF = reason => {
			showError(
				'Impossibile ottenere le Statistiche di Gioco, riprovare più tardi',
				['Impossibile ottenere le Statistiche di Gioco, reason: ', reason],
				true
			)
		}
	...
	startGame() {
		...
		let secretError = () => {
			showError(
				'Impossibile iniziare la partita, riprovare più tardi',
				['Impossibile iniziare la partita: ottenuta risposta', xhr],
				true
			)
		}
	...
	tryExists(callback) {
		...
		let checkError = () => {
			...
			showError(
				`Errore connessione, riprovare più tardi`,
				['Errore controllo tentativo: ottenuta risposta', xhr],
				false
			)
			callback(false)
		}
	...
	updateStats() {
		const errF = (reason) => {
			showError(
				'Impossibile aggiornare i dati, riprovare più tardi',
				['Impossibile aggiornare i dati, ottenuta risposta: ', reason],
				true
			)
		}
